...
dplyrXdf supports all the table-join verbs from dplyr, except for the set operations `intersect` and `setdiff`. The syntax is the same as for the dplyr versions, including joining on non-matching column names. The underlying implementation uses `rxMerge` with the appropriate arguments for each type of join.
