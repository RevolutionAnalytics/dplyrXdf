...
In addition, dplyrXdf provides methods for the following utilities:

* `cbind` and `rbind` to combine Xdf files
* `as.data.frame`, `collect` and `compute` to convert an Xdf file into a data frame
* `[[`, `$` and `pull` to extract a column into memory
* `sample_frac` and `sample_n` to do random sampling
...
For the moment, dplyrXdf only supports files stored in the local filesystem. Support for datasets stored in HDFS (using the `RxHdfsFileSystem` class) may appear in a future version. For data stored in a SQL database, consider using the dplyr backend for that database, if available --- such as the RSQLServer package mentioned at the start of this vignette.


## Conclusion

This article has been a quick executive-summary introduction to dplyrXdf. If you have any suggestions on features to add (including bits of dplyr that have been left out) or bugs that need fixing, please contact me at [hongooi@microsoft.com](hongooi@microsoft.com).
