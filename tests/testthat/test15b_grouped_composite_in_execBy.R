context("Grouped composite Xdf input, execBy")
...
{
    tbl <- mtc %>% group_by(gear) %>% filter(mpg > 15, cyl <= 6, .outFile="test15a")
    expect_true(verifyCompositeData(tbl, "RxXdfData"))
    tbl <- mtc %>% group_by(gear) %>% select(mpg, cyl, drat, .outFile="test15a")
    expect_true(verifyCompositeData(tbl, "RxXdfData"))
    tbl <- mtc %>% group_by(gear) %>% subset(mpg > 15, c(mpg, cyl, drat), .outFile="test15a")
    expect_true(verifyCompositeData(tbl, "RxXdfData"))
    tbl <- mtc %>% group_by(gear) %>% mutate(mpg2=sin(mpg), wt2=sqrt(wt), .outFile="test15a")
    expect_true(verifyCompositeData(tbl, "RxXdfData"))
    tbl <- mtc %>% group_by(gear) %>% transmute(mpg2=sin(mpg), wt2=sqrt(wt), .outFile="test15a")
    expect_true(verifyCompositeData(tbl, "RxXdfData"))

    tbl <- mtc %>% group_by(gear) %>% summarise(n=n(), mpg2=mean(mpg), .outFile="test15a")
    expect_true(verifyCompositeData(tbl, "RxXdfData"))

    tbl <- mtc %>% group_by(gear) %>% factorise(cyl, gear, .outFile="test15a")
    expect_true(verifyCompositeData(tbl, "RxXdfData") && varTypes(tbl)["cyl"] == "factor")
    tbl <- mtc %>% group_by(gear) %>% distinct(cyl, gear, .outFile="test15a")
    expect_true(verifyCompositeData(tbl, "RxXdfData"))
    #tbl <- mtc %>% group_by(gear) %>% rename(mpg2=mpg, .outFile="test15a")
